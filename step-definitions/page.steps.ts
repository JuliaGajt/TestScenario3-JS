...
Then(/^"(.*)" application is visible$/, async (app) => {
  ...
  expect(
    await (await setupPage.actualApplicationName(app)).isDisplayed()
  ).equals(true);
});
...
Given(/I am on Accounts page$/, async () => {
  ...
  await (
    await accountPage.recordNameHeader(accData.accountName)
  ).waitForDisplayed();
});

Then(
  /(.*) named "(.*)" can be found by Gloabl Search$/,
  async (objectType: string, name: string) => {
    await homePage.searchInGlobal(name, objectType);
    await browser.waitUntil(async () => {
      return (await searchResultPage.searchResultsHeader).isDisplayed();
    });
    await expect(
      await (await searchResultPage.resultsGlobalSearch(name)).getText()
    ).equals(name);
  }
);
