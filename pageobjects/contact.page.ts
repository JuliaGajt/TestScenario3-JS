...
class ContactPage extends ObjectPage {
  ...
  get stageDropDownBtn() {
    return $(
      `//span[text()='Stage']/parent::span/following-sibling::div/div/div/div/a[@class='select']`
    );
  }
  get accountNameLookupInput() {
    return $(
      `//span[text()='Account Name']/parent::label/following-sibling::div//input`
    );
  }
  async accountNameLookupSelect(accountName: string) {
    return $(`//div[@title="${accountName}"]/parent::div/parent::a/parent::li`);
  }

  stageOptionBtn(stage: string) {
    return $(`//a[text()='${stage}']/parent::li`);
  }
  linkToNewOpp(oppName: string) {
    return $(`//a[@title='${oppName}']`);
  }

  get newOpportunityBtn() {
    return $(
      `//div[contains(@class,'active')]//button[@name="Global.NewOpportunity"]`
    );
  }
  get saveBtn() {
    return $(
      `//div[contains(@class,'active')]//footer/button/span[text()="Save"]`
    );
  }
  ...
  async createNewOpportunity(opportunity: Opportunity) {
    ...
    await (
      await this.closeDateInput
    ).setValue(opportunity.closeDate.toLocaleDateString());
